...
const server = http.createServer(async (req, res) => {
  ...
  // ---- Public lookup by CNIC number (no login needed) ----
  if (pathname.startsWith("/api/lookup/") && req.method === "GET") {
    const cnicRaw = decodeURIComponent(pathname.split("/api/lookup/")[1] || "");
    const cnic = cnicRaw.trim();
    const row = db.prepare("SELECT * FROM submissions WHERE cnic = ? ORDER BY serial_number DESC LIMIT 1").get(cnic);
    if (!row) return sendJSON(res, 404, { ok: false, error: "No record found with that CNIC number" });
